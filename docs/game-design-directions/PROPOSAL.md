...
> **主论点演化（P0–P2.5 后）**：从"给技能回报排名"转为**"技能回报是一个可测但被误测
> 的设计属性，且与机制难度分离"**。核心贡献 = 测量方法论（三种操作化不相关、win-vs-tier
> 是弱测度）+ 设计分离发现（回报 ≠ 难度）+ 感知（anti-fun ≠ overpowered）。

## 研究问题

- **RQ1（技能表达）**：每个英雄的性能（胜负贡献、对线经济/经验差、KDA）如何随操作者
  段位变化？据此给英雄的"精通回报 / skill-reward"排名。它与官方"预期难度"标签一致吗？
  **（答案：不一致——回报与难度分离；且 win-vs-tier 本身是被混杂的弱测度。）**
...
**⚠️ 稳健性检查后降级（2026-07-16）**：用 P2.5 角色分层放大重算 RQ3 →
Spearman 0.14，**CI [−0.04, 0.32] 含 0**，不再显著；位置内异质
（top +0.32 / adc +0.37 / mid +0.08 / jungle −0.14 / support −0.23，各组 n 小）。
**RQ3 定级为探索性发现**：个案（锴）与对线位存在迹象，但非普适效应；
论文已相应改写（摘要 + 第 6 节）。证实/证伪需要后续时代的分位置 ban 数据。

### 外部验证（Riot 官方难度标签，Data Dragon info.difficulty 1–10）——弱/不显著，但富信息

- P1 对线放大 vs Riot 难度：**Spearman 0.09，95% CI [−0.11, 0.27]（含 0）**；剔除 21 个
  2016 后重做英雄后 0.12，仍不显著。P0 段位斜率 vs 难度 ≈ 0。
- **诊断出角色混杂**：难度高但放大低的多是辅助/刺客/游走（Bard、Fiddlesticks、Shaco、
  Katarina——对线差不反映其技能）；难度低但放大高的**几乎全是打野**（Rek'Sai、Olaf、
  Nunu、Vi、Nocturne——lane_diff 对打野是对拼野区/入侵，性质不同）。→ **lane_diff 放大
  测度有角色敏感性，打野系统性偏高。**
- **两条并存的诚实结论**：(1) **机制难度 ≠ 技能回报**——Riot 的复杂度标签与实测技能
  放大是不同构念（本身是一个可主张的设计发现）；(2) 我们的放大测度需**按角色分层**
  才可信。外部验证目前**不确定**，非干净背书。

### 对论文定位的影响（诚实）

三个测度（win-vs-tier、lane 执行放大、Riot 难度）**两两弱相关甚至零相关**。这把论文的
重心从"给出技能回报排名"推向**测量方法论贡献**："英雄技能表达的操作化是微妙的——
胜率-段位被匹配压缩+选择混杂、对线执行放大有角色混杂、都不与官方机制难度对齐；
这些是不同构念。"这更诚实、更站得住（FDG/ToG 的测量类贡献），但不再是"一张漂亮排名图"
的故事。**下一步 P2.5（角色分层）是让技能测度可信的关键。**

## P2.5 结果（2026-07-15，`analysis/design_p2_5.py`）：角色分层——收束为核心发现

把对线压制指标**在位置内 z 标准化**后重算技能放大（辅助只和辅助比、打野只和打野比）。

- **验证仍为零，但这次是决定性的**：位置控制后 amp vs Riot 难度偏相关 **−0.03**；
  各位置内也都近 0（top −0.02 / mid 0.02 / adc 0.01 / jungle −0.03 / support −0.23）。
  → 之前的弱相关**不是角色混杂造成的假象，而是真实的构念分离**。
- **但分位置排名在角色内高度可信**（role-fair）：
  - mid：Lissandra、Viktor、Xerath、Kassadin、Azir、LeBlanc（高操作法师/刺客）；
  - jungle：Rek'Sai、Nunu、Hecarim、Nidalee、Elise、Nocturne（机制打野）；
  - adc：Sivir、Kog'Maw、Quinn、Corki、Kalista；top：Jax、Cho'Gath、蒙多、茂凯（滚雪球/
    续航上单）；support：Taric、Lulu、Zilean、Braum。

**核心设计发现（论文主论点，现在站得住）**：**技能"回报"（精通是否转化为资源/胜势）
与机制"难度"（Riot 复杂度标签）是可分离的、不同的设计轴**——即使角色分层后依然分离。
测度是 face-valid 的（角色内能还原公认的吃操作英雄），但它测的不是"难不难操作"，而是
"操作得好回报大不大"。滚雪球/续航型（Jax、Cho'Gath）和高机制型（Azir、LeBlanc）都能
奖励技能，但前者 Riot 难度低——这解释了零相关。
...
- [x] P2 分段位感知失衡 + bootstrap CI + RQ3 交互（弱但显著正相关）
      ...
      - [x] Riot 难度外部验证已补跑：弱/不显著（0.09），诊断出角色混杂 + "机制难度≠技能回报"。
- [x] **P2.5**：位置内标准化的技能放大；验证仍零 → **技能回报与机制难度分离**（核心发现）；
      分位置排名 role-fair 可信。实证部分（P0–P2.5）至此完整。
- [ ] P3 写作。定位收敛为：**测量方法论 + 设计分离发现**——
      (1) "技能表达"三种操作化（win-vs-tier / 对线执行放大 / Riot 难度）两两不相关，
      win-vs-tier 被匹配压缩+选择混杂；(2) 技能回报 ≠ 机制难度（可分离设计轴，角色分层后仍成立）；
      (3) 感知（ban）：anti-fun ≠ overpowered，奖励精通者在高分段弱显著地更被 ban（RQ3）。
      目标 FDG / IEEE ToG（测量类贡献）。
